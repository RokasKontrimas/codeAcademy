...
createElementList(task20, 1)

// 7.21
let task21 = posts.map((post) => {
    return `Title: ${post.title}. Content: ${post.body}`;
});
createElementList(task21, 2)

// 7.22
let task22 = posts.map((post) => {
    return post.title;
}).filter(title => title[0] === "s");
createElementList(task22, 3)

// 7.23
let task23 = posts.map((post) => {
    return post.title;
}).filter(title => title[title.length - 1] === "t" || title[title.length - 1] === "m");
createElementList(task23, 4)

// 7.24
let task24 = posts.map((post) => {
    return post.title;
}).filter(title => title.length > 15);
createElementList(task24, 5)

// 7.25

let task25 = posts.map((post) => {
    return [post.title, post.body];
}).filter(pair => {
    return pair[0].length < 20 && pair[1].length > 50;
});
createElementList(task25, 6);


// 7.26

let task26 = posts.map((post) => {
    return [post.title, post.body];
}).filter(pair => {
    let title = pair[0];
    let body = pair[1];
...
createElementList(task26, 7);

// 7.27
let task27 = posts.map((post) => {
    return [post.title, post.body];
}).filter(pair => {
    let title = pair[0];
    let body = pair[1];
...
createElementList(task27, 8);

// 7.28

let task28 = posts.map((post) => {
    return post.body
}).filter(post => {
    return post.includes('sit') && !/\bsit\w*/.test(post)
})
createElementList(task28, 9)


// 7.29
let task29 = posts.map((post) => {
    return [post.title, post.body]
}).filter(pair => {
    let title = pair[0];
    let body = pair[1];
...
createElementList(task29)

// 7.30
let task30 = posts.map((post) => {
    return [post.body, post.body.length]
})
createElementList(task30)
